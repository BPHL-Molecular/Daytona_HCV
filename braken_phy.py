...
with open("./output/sum_report.txt", 'r') as kreport:
        ...
        for l in lines[1:]:
            ...
            if(substring.lower() in tax.lower()):
                ...
                os.system(f"singularity exec docker://staphb/samtools:1.12 samtools mpileup -A -B -d 8000 --reference {current_dir}/reference/hcv/KC248195.fasta -Q 0 {current_dir}/output/extract/{sampleID}_{taxID}.sorted.bam | singularity exec docker://staphb/ivar:latest ivar consensus -t 0 -m 10 -n N -p {current_dir}/output/extract/{sampleID}_{taxID}.consensus")
...
if args.phy == 1:
    os.system(f"singularity exec docker://staphb/mafft:latest mafft --addfragments {current_dir}/output/extract/sum_consensus.fa {current_dir}/reference/HCV_align/hcv_7genotypes_aln > {current_dir}/output/extract/mafft_genotype_msa")
    ...
    os.system(f"singularity exec docker://staphb/mafft:latest mafft --addfragments {current_dir}/output/extract/sum_consensus.fa {current_dir}/reference/HCV_align/hcv_allsubtypes_aln > {current_dir}/output/extract/mafft_subtype_msa")
    ...
    os.system(f"singularity exec docker://staphb/phytreeviz:latest phytreeviz -i {current_dir}/output/extract/SNPs_boot_subtype.contree -o {current_dir}/output/extract/SNPs_boot_subtype.contree.pdf --show_confidence")
    os.system(f"singularity exec docker://staphb/phytreeviz:latest phytreeviz -i {current_dir}/output/extract/SNPs_boot_subtype.contree -o {current_dir}/output/extract/SNPs_boot_subtype.contree.png --show_confidence")
elif args.phy == 3:
    os.system(f"singularity exec docker://staphb/mafft:latest mafft --addfragments {current_dir}/output/extract/sum_consensus.fa {current_dir}/reference/HCV_align/hcv_7genotypes_aln > {current_dir}/output/extract/mafft_genotype_msa")
    os.system(f"singularity exec docker://staphb/snp-sites:2.3.3 snp-sites -o {current_dir}/output/extract/SNPs_genotype.fasta -c {current_dir}/output/extract/mafft_genotype_msa")
    os.system(f"singularity exec docker://staphb/snp-dists:0.6.2 snp-dists {current_dir}/output/extract/mafft_genotype_msa > {current_dir}/output/extract/pairwise_matrix_genotype.tsv")
